...
public class PROWAlgorithm extends Algorithm {
	...
	private boolean pairRemoved = false;

	public PROWAlgorithm() {
		super();
	}
	...
	public void buildCombinations() {
		...
		if (sameNumOfValues() && !pairRemoved){
			
			int times = 0;
			if (this.sets.size() != 4){ 
				times = this.sets.size() / 4;
			}
			if (times >0){
				System.out.println("IGUAL NUM VALORES, MULTIPLO 4");
				int visits=0;
				Combination c = null;
				Vector <Combination> combs = this.buildCombOf4();
				this.initPairTable(); //Esto no se que hace, lo pase a una funci—n
				for (int i=0; i<combs.size(); i++){
					//Primero ver si la comb es valida (FALTA!!!)
					c = complete (combs.get(i));
					visits = this.addCombination(c); //Nueva funcion
				}
				 //A partir de aca copiado de build combinations
				Pair p;
				c = null;
				...
				System.out.println("IGUAL NUM VALORES, CUATRO o MENOR");
				buildCombinationsSameNumValues();
			}
		} //if de times
		else{ //if sameNumOfValues
			System.out.println("DISTINTO NUM VALORES");
			initPairTable();
			System.out.println("AL INICIO\t"+this.visitedPairs.toString());
			buildNextCombinationsBestIndex();
	...
	public void buildNextCombinationsBestIndex(){
		int visits=0;
		Pair p;
		Combination c=null;
		do {
			...
			c.setValuePosition(bestSetAndIndex[0], bestSetAndIndex[1]);
			c=completeDiffValues(c);
			visits = this.addCombination(c);
			if (c==null || visits ==0 && ((p=getPairWithWeight0(pairsTables))!=null)){
				c=null;
	...
	public boolean sameNumOfValues(){
		int  valaux = 0;
		int values = 0;
		boolean seguir= true;
		if (this.sets.size()>0){
			values = this.sets.get(0).size();
		}
		for (int t=0; (t<this.sets.size() & seguir); t++){
			valaux = this.sets.get(t).size();
			if (values != valaux){
				seguir = false;
			}	
		}
	...
	private Combination completeDiffValues(Combination c) { 
		//Completa un caso de prueba
		for (int i=0; i<this.sets.size(); i++) {
			if (c.getPosition(i)==-1) {
				int selectedElementPosition=selectElementVisitingMoreCompatiblePairsDiffValues(i, c);
	...
	private int selectElementVisitingMoreCompatiblePairsDiffValues(int setIndex, Combination c) {
		...
		for (int i=0; i<vp.size(); i++) { //i toma los valores de setIndex
			...
			if (p!=null && (vp.get(i)>max)) {
				if (valueIsCompatible(c, setIndex, i)) {
					result=i;
					max=vp.get(i);
					MAXIMO_VISITAS=visitas;
					MAXIMO_PESO=peso; //FALTABA
				}
	...
	public Vector <Combination> buildCombOf4 () {
		int times = this.sets.size() / 4;
		Vector <Combination> combs = new Vector <Combination>();
		if (times >0){
			for (int ind=0; ind<times; ind++){
				//Busco las combinaciones para eso 4 parametros
				PROWAlgorithm3 a=new PROWAlgorithm3();
				int desde = ind*4;
				for (int i=desde; i<desde+4; i++){
					 SetCustom conj=this.sets.get(i);
					 a.add(conj);
				}
				a.buildPairTables();
				a.buildVisitedPairs();
				...
				if (ind ==0){
					int size = a.getprowCombinations().size();
					for (int r=0; r<size; r++) {
						long pos = a.getprowCombinations().get(r);
						Combination aux = a.getCombination(pos);
						Combination c=new Combination(this.sets.size());
						for (int i=0; i<this.sets.size(); i++)
							c.setValuePosition(i, -1);
						int it=0;
						for (int j=desde; j<desde+4; j++){
							c.setValuePosition(j,aux.getPosition(it));
							it++;
						}
						combs.add(c);
					}
				}else{
					System.out.println("Entro en ind: " + ind);
					int hasta = combs.size();
					int size = a.getprowCombinations().size();
					if (hasta==size){ //los conjuntos son de igual tama–o
						for (int r = 0; r<hasta; r++){ //por cada combinacion en combs
							int it=0;
							long pos = a.getprowCombinations().get(r); //tomo la posicion 
							Combination aux = a.getCombination(pos); //tomo la combinacion 
							for (int j=desde; j<desde+4; j++){
								combs.get(r).setValuePosition(j,aux.getPosition(it));
									it++;
							}
							System.out.println(combs.get(r).toStringPositions());
						}
					}
	...
	public void showCombinations(){
		int size = this.getprowCombinations().size();
		for (int r=0; r<size; r++) {
			long pos = this.getprowCombinations().get(r);
			Combination aux = this.getCombination(pos);
		}
	...
	public void buildCombinationsSameNumValues() {
	...
	public void buildCombinationsDiffValuesLess4() {
		//Construye las combinaciones de menos de 5 valores con diferentes num de valores
		this.initPairTable(); //Esto no se que hace, lo pase a una funci—n
	...
	private Combination initializeNewCombination() { //CAMBIADOOOOOO
		...
		if (sameNumOfValues() && this.sets.size()==4){
	...
	private Combination combinationZero() { //NUEVO
		...
		for (int i=0; i<this.sets.size(); i++){
			...
			do {
				j++;
				salir = valueIsCompatible(selected, i, j);
			} while ((!salir) && (j < this.sets.get(i).size()));
			if (salir){
				selected.setValuePosition(i, j);
			}
		}
		return selected;
	}
